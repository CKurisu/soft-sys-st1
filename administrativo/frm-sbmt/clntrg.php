...
try {
    ...
    $statement=$connectionPDO->prepare("SELECT * FROM {$tbl_clinter} WHERE Laboratorio=:lab AND Nombres=:nom AND ApPaterno=:app AND ApMaterno=:apm AND Correo=:mail AND Direccion=:add AND Telof=:tel AND Extension=:ext AND Celular=:cel LIMIT 1;");
    ...
    if(!($row)){
        $trcli=$connectionPDO->prepare("INSERT INTO {$tbl_clinter} VALUES (:lab, :nom, :app, :apm, :mail, :add, :tel, :ext, :cel, 1, :usr, :dt, :usr, :dt);");
        ...
        $trcli->bindParam(':dt', $x1, PDO::PARAM_STR);
        $stln->execute();
