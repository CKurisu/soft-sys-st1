        ...
        case 'mat':
            try {
                if(empty($_POST['x1'])  ||
                    ...
                    empty($_POST['x16']))
                   {
                        $message=$html->warning('Compruebe que no existan campos vacios');
                   }    
                    $lab=$_POST['x1'];
                    $tpcli=trim($lab);
                    $rznscl=$_POST['x2'];
                    ...
                    $dt= date('Y-m-d');
                    $stcli=$cnxPDO->prepare("SELECT * FROM {$tbl_cli} WHERE CliLab=:lab OR RznScl=:rzn OR TpCli:tp LIMIT 1;");
                    $stcli->bindParam(':lab', $lab, PDO::PARAM_STR);
                    $stcli->bindParam(':rzn', $rznscl, PDO::PARAM_STR);
                    $stcli->bindParam(':tp', $tpcli, PDO::PARAM_STR);
                    $stcli->execute();
                    $rowcli=$stcli->rowCount();
                    $stclisl=$cnxPDO->prepare("SELECT * FROM {$tbl_dtcli} WHERE RFC=:rfc AND Calle=:str AND NumExt=:numex AND NumInt=:numin AND Col=:col OR CP=:cp OR Loc=:loc LIMIT 1;");
                    ...
                    if((!($rowcli))&&(!($rowclisl))){
                        $trclirg=$cnxPDO->prepare("INSERT INTO {$tbl_cli} VALUES (NULL,:lab,:rzn,:idint,1,:usr,:dt,:usr,:dt,:tp);");
                        ...
                        $stclirg->bindParam(':tp', $tpcli, PDO::PARAM_STR);
                        $trclirg->execute();
                    ...
                    $cnxPDO->rollBack();
                    $message=$html->danger($ex->getMessage());$fail=1;
                }
            break;
        case 'suh':
            try {
